...
| R12 | **Nothing authorises traffic inside the cluster.** Any pod can call `ornith-9b-predictor` with no token and no quota | `git grep 'kind: AuthorizationPolicy\|kind: NetworkPolicy'` returns nothing, 2026-08-20. Namespace `llm` is ambient (`platform/10-istio/namespace-llm.yaml:14`), which gives mTLS, and mTLS is not authorisation. Both Kuadrant policies sit on an HTTPRoute, so they see gateway traffic only | **Confirmed on a live cluster 2026-08-20.** From a Pod in namespace `llm` with no token: `GET /v1/models` returned HTTP 200 serving `ornith-9b`, and `POST /v1/chat/completions` returned HTTP 200, a real inference charged to nobody. `security/oidc/authorizationpolicy.yaml` is written and **not yet applied**: allow the gateway's and Prometheus's ServiceAccounts, deny the rest. L4 is all ambient enforces without a waypoint, and the engine serves `/metrics` and `/v1` on one port, 8080, so such a policy cannot tell a scrape from an inference call. Accept that, or pay for a waypoint |
