...
| R19 | **Any restart can take the whole service down until a human restarts the gateway.** Envoy fetches the Kuadrant Wasm shim once at startup, fails closed, and never retries | **Measured on a live cluster 2026-08-20**, eight hours into a healthy run, with no bootstrap involved. The Kuadrant operator container died at 13:36:45Z and came back at 13:37:06Z. The gateway exhausted its fetch retries at **13:36:52.999Z**, inside that 21-second window: `Retry limit exceeded for fetching data from remote data source`, then `Plugin kuadrant-wasm-shim failed to load`, then `Plugin configured to fail closed failed to load`. The Service endpoint was healthy again 13 seconds later and Envoy never looked. Every request returned **503**, token issuance included, because Keycloak is behind the same gateway. `kubectl rollout restart deploy/llm-istio` fixed it in one attempt | **Fixed 2026-08-20**, and proved by mutation twice. `platform/10-istio/wasm-watchdog.yaml` is a CronJob that reads the one metric that discriminates the states and patches the Deployment's pod template, which is what `kubectl rollout restart` does. A probe would have been better and is not available: the Deployment is owned by the Gateway and reverted by `istio.io/gateway-controller`, and the `EnvoyFilter` is labelled `kuadrant.io/managed=true` with no `retry_policy` to widen. Reproduced by deleting the Kuadrant operator Pod and restarting the gateway in the same instant. **Run 1:** broke 15:04:28Z, watchdog held off at 15:04 and 15:06 because the observation window was not full, patched at 15:08:01Z, HTTP 401 by 15:08:15Z. **Run 2:** broke 15:16:38Z, patched 15:20:01Z, back by 15:20:19Z. So 3m47s and 3m41s unattended, against unbounded before. Watched for six minutes and four further runs afterwards: `generation` stayed at 7 and the annotation did not move, so one failure gives exactly one restart. Two samples are a measurement and not a guarantee |
...
| R21 | **The token quota is opt-in by the caller.** A streamed response that does not ask for `usage` is counted as zero, so the whole cost-control layer is bypassed by a default client | **Measured 2026-08-21**, same tier, same 711-token prompt against a 500-token budget, same 15s client cut, only `stream_options.include_usage` differing. **With** it: HTTP 200 in 2.6s, next request **429**. **Without** it: HTTP 200, next request **200**, and a third **200**. The gateway's own log says why, once per streamed request: `kuadrant_wasm_shim: Missing json property: /usage/total_tokens` then `Task failed: Some("4")`. `tests/smoke/06-auth-quota.bats` passes because its `chat()` does not stream, so criterion 4 is verified on a path no interactive client here used | **Half fixed 2026-08-21.** `tools/chat.sh`, `platform/30-observability/ttft-prober-cronjob.yaml`, `bench/recovery-drill.sh`, and `tests/smoke/08-availability.bats` now set `include_usage`; `bench/harness.py` already did. **That fixes this repository's clients and not the risk.** A caller who omits the field still pays nothing, and a caller who wants free tokens will omit it. The quota is only a quota when the counting does not depend on the request. Fixing that needs Kuadrant to count what it can see, or the gateway to inject the field, and neither is settled here |
| R22 | **The shim holds a streamed response open long after the body is complete**, whenever `usage` is absent | Measured 2026-08-21, identical small requests through the gateway, only `include_usage` differing: **without** it `ttfb=2.38s total=40.93s`; **with** it `ttfb=1.92s total=1.92s`, so the response closes on the last byte. A big-prompt run delivered all four chunks ending in `data: [DONE]` and then held the connection past a 120s and a 420s client timeout. The `activeDeadlineSeconds: 45` on the TTFT prober sits inside that window, which is the likeliest reading of the prober Pods `docs/STATUS.md` records in `Error` | Fixed for this repository's clients by the same change as R21, and the underlying hold is not fixed. Any caller omitting the field still pays it. Worth knowing before trusting any latency number taken through the gateway by a client that omits the field |
| R23 | **`tools/chat.sh` sent an unbounded request**, against this repository's own CI lesson | Read 2026-08-21: line 53 was `curl -sN -X POST` with no `--max-time`, while `docs/STATUS.md` states the rule as "give every request a `--max-time` and every loop a wall clock". Observed the same day: `task chat` with a 700-token prompt and no `max_tokens` ran past eight minutes. That was generation at 0.2 tokens per second, not a hang, and an unbounded `curl` is exactly what cannot tell the two apart | **Fixed 2026-08-21.** `--max-time 600`, generous on purpose because the bound is there to end a hang, not to cap an answer. A second defect in the same file stays open: `docs/STATUS.md` records that it printed `Internal Server Error.` and exited 0 |
| R24 | **No rolling update of the predictor can ever complete**, and Argo CD reports `Synced` and `Healthy` throughout | **Measured 2026-08-21** on the first push that changed the predictor's Pod template. `maxSurge` is 25% of 2, which rounds **up** to 1; `maxUnavailable` is 25% of 2, which rounds **down** to 0. So the Deployment must add a Pod before removing one, and the new Pod cannot schedule: `0/3 nodes are available: 1 node(s) had untolerated taint(s), 2 node(s) didn't match pod topology spread constraints`. That is R8's constraint, reached without KEDA. Deleting an old Pod does not help: the old ReplicaSet refills it, because `maxUnavailable: 0` forbids scaling it down. Throughout, `model-local` read `Synced` and `Healthy`, the `ServingRuntime` on the cluster carried the new spec, and the Pod serving traffic carried the old one | None yet, and it needs one of R8's three choices rather than a fix of its own. This is R1's shape again: the Application is green, the object is updated, and the workload is not. Worth knowing before believing that a pushed change to `runtimes/` or `models/` is running |
| R25 | **R13's own fix failed open, and only a live test found it.** A quota counter keyed on a claim the `AuthPolicy` does not export counts nothing at all | **Measured 2026-08-21, after the fix was pushed.** The same test that returned 429 with the counter on `tier` at 23:02Z returned **200** with it on `azp` at 23:51Z, and a third request also 200. The gateway said why on every request: `CelError::Resolve { NoSuchKey("azp") }`. Only what `authpolicy.yaml`'s `response.success.filters` exports is visible to the CEL in `tokenratelimitpolicy.yaml`. `azp` is in the token, and being in the token is not enough | **Fixed 2026-08-21** by exporting `azp` alongside `tier`, verified by spending a budget: 711 tokens returned 200 and the next request 429, with zero `NoSuchKey` lines. **CI caught this without being asked to.** Run `32429474915` failed on `tests/smoke/06-auth-quota.bats` test 30 with forty consecutive 200s in 16 seconds, which is the same fail-open, found from a real cluster it built itself. So the gap was never a missing test. The gap was pushing a change that a marker in the manifest had already flagged as untried, and learning the answer from CI instead of from the ten minutes it would have cost to spend a budget first. Two things to keep: **a rate-limit expression that cannot resolve fails open, not closed**, and the window between a push and a CI verdict is a window in which the quota was not enforced |
...
**R21 is the most important row in this file, and it was found in a log line
nobody was reading.** The three log lines that produced it had been in the gateway
for as long as the gateway had been serving streamed requests. They were seen
while chasing R19, which is a different defect in a different component. Nothing
in this repository was watching for them, `task chat` looked like it worked, and
the quota test passed on the one path that was never affected.
...
**R19's fix is the first thing in this file that was designed on a live cluster
rather than verified on one, and two defects in it were caught that way.** The
first version's guard did not do what its comment claimed: `max_over_time == 0`
is true after a single zero sample, so it would have restarted a healthy gateway
one second before it finished loading. The second version reported
"every gateway pod has loaded the shim" while the gateway was returning 503,
because a query that matched nothing and a cluster that was healthy produced the
same empty result. Both were found by running the thing, not by reading it.
