...
| R16 | **A model fact lives in `runtimes/`.** The ServingRuntime's memory sizing was chosen for this model | `runtimes/llamacpp-arm64/servingruntime.yaml:74` requests 8Gi and limits 12Gi, sized for a 9B Q4 model with `--ctx-size 4096`. The proof that it is a model fact is `models/ornith-9b/overlays/local/fallback.yaml:64`, where `fallback-small` has to override it to 1Gi and 2Gi | Move `resources` into the model overlay. `--ctx-size` and `--parallel` deserve the same treatment, for the same reason |
| R17 | **Fetching the weights has no wall clock, and its source is a moving reference** | `models/ornith-9b/overlays/local/patch-resources.yaml:158` runs `curl -fL --retry 3` with no `--max-time` and no `--speed-limit`, so a stalled transfer hangs the init container with no signal but a pod stuck in `Init`. The URL uses `resolve/main`, a mutable ref; `sha256sum -c` makes a changed file fail closed rather than serve wrong, so every replica CrashLoops at once instead | Add both bounds: this repository's own CI lesson, "give every request a `--max-time` and every loop a wall clock", was never applied here. Separately, `docs/deployment-walkthrough.md:233` records that each replica downloads 5.6 GB independently, so a KEDA scale-up cannot answer a spike: `scaledobject.yaml:33` polls every 15s and a new replica needs minutes. Cache the file across pod lifetimes, as [the recovery drill runbook](../runbooks/recovery-drill.md) already proposes |
| R18 | **One alert exists in the whole repository, and it watches the prober** | `git grep -c 'alert:'` finds exactly one, `TTFTProberStale` at `platform/30-observability/recording-rules.yaml:197`. `llmstack:gateway_error_ratio:rate5m` is recorded at line 164 and nothing reads it, so the one failure mode this repository has actually measured, R7, would raise nothing | Three alerts: the gateway 5xx ratio, a queue that stays above the KEDA threshold, and `spec.replicas` differing from `readyReplicas`. The third would catch R8, which `tests/smoke/07-autoscaling.bats` passes straight through |
...
would turn R8 from a silent pass into an alert.
