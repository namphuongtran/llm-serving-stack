...
| R25 | **R13's own fix failed open, and only a live test found it.** A quota counter keyed on a claim the `AuthPolicy` does not export counts nothing at all | **Measured 2026-08-21, after the fix was pushed.** The same test that returned 429 with the counter on `tier` at 23:02Z returned **200** with it on `azp` at 23:51Z, and a third request also 200. The gateway said why on every request: `CelError::Resolve { NoSuchKey("azp") }`. Only what `authpolicy.yaml`'s `response.success.filters` exports is visible to the CEL in `tokenratelimitpolicy.yaml`. `azp` is in the token, and being in the token is not enough | **Fixed 2026-08-21** by exporting `azp` alongside `tier`. The lesson is the one worth keeping: the `Untried` marker on R13 named exactly this and it was pushed anyway, so the marker was doing its job and nobody acted on it. **A rate-limit expression that cannot resolve fails open, not closed.** Anything that changes a counter or a predicate here has to be tested by spending a budget, on a cluster, before it is pushed |
