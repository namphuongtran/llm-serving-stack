...
**R11, R12, and R15 were left alone on purpose, and the reason is not effort.**
All three change the request path: the issuer Authorino trusts, who may reach
the predictor, and which object the policies attach to. None can be verified
without a cluster, and getting one wrong is worse than the risk it closes, since
the failure modes are an open gateway or a service that answers nothing. They
are the next thing to do on a live cluster, in that order, each followed by the
2026-08-20 load test rather than by a single request.
