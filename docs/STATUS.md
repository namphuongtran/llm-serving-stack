...
| 4 | Exceeding the token quota returns 429 | **HOLDS 2026-08-20** for a request that does not stream, in CI **and locally**. **It does not hold for a streamed request that omits `stream_options.include_usage`**, which is counted as zero; measured 2026-08-21, see R21. `tests/smoke/06-auth-quota.bats` does not stream, which is why it passes. This row also said "not reachable on the local engine" for a day; see finding 4 | `bats tests/smoke/06-auth-quota.bats` |
